...
> Codex / Claude Code 接手开发的入口文档。

## 项目定位

独立的多项目开发监控 Web 面板。监控 continuous-claude-deepseek 在多个项目上的自主开发进度（PR、任务完成度、花费、CI 状态）。
...
- Cost 数据: 解析 `logs/claude-stdout-*.log` 中的 JSON（total_cost_usd）
